function [bestF, bestP, bestR, bestT, F_max, P_max, R_max, Area_PR] = collect_eval_bdry1(pbDir,fileNum)
...
% Updated by David Stutz <david.stutz@rwth-aachen.de>
    ...
    txtName = strcat(fileNum,'_eval_bdry.txt');
    ...
    if length(dir(fname)) == 1
        ...
        txtName = strcat(fileNum,'_eval_bdry_img.txt');
        fname = fullfile(pbDir,txtName);
        fid = fopen(fname,'w');
        if fid==-1
        ...
        % the id of best value
        txtName = strcat('id_best_bdry.txt');        
        fname = fullfile(pbDir,txtName);
        dlmwrite(fname,id_best,'-append');
