function [thresh,cntR,sumR,cntP,sumP] = evaluation_bdry_imageSP(inFile,gtFile, prFile, nthresh, maxDist, thinpb)
...
% Updated by David Stutz <david.stutz@rwth-aachen.de>
    ...
    [p,n,e]=fileparts(inFile);  % seg mat file
    if strcmp(e,'.mat')
        load(inFile);
    end;
    ...
    if ~exist('superpixel', 'var')
        ...
        if nthresh ~= numel(superpixel)
            %warning('Setting nthresh to number of segmentations');
            nthresh = 1;  % the number of segs 5
        end;
    ...
    for t = 1:nthresh
        ...
        if ~exist('superpixel', 'var')
            ...
            bmap = logical(seg2bdry(superpixel,'imageSize'));
        end;
