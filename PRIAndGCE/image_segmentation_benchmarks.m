...
for imageIndex=1:length(testImageList)
    ...
    [benchX, benchY] = size(groundTruth{1}.Segmentation);
    
    for benchIndex=1:length(groundTruth)
        benchLabels = groundTruth{benchIndex}.Segmentation;
...
%         end
        ...
        [curRI,curGCE,curVOI] = compare_segmentations(double(sampleLabels),double(benchLabels));       
        sumRI = sumRI + curRI;
    ...
    averageBoundaryError = averageBoundaryError + totalBoundaryError / length(groundTruth);
    averageRI = averageRI + sumRI / length(groundTruth);
    averageVOI = averageVOI + sumVOI / length(groundTruth);
    averageGCE = averageGCE + sumGCE / length(groundTruth);
